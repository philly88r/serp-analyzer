...
class SerpAnalyzer:
    ...
    async def search_google(self, query, num_results=6):
        ...
        async with AsyncWebCrawler() as crawler:
            # In version 0.4.247, we pass parameters directly to arun
    ...
    async def analyze_page(self, url):
        ...
        async with AsyncWebCrawler() as crawler:
            ...
            # In version 0.4.247, we need to use html instead of text
            content_text = soup.get_text()
            word_count = len(content_text.split())
            
            # Get clean markdown content
            # In version 0.4.247, markdown might be a string or have a different structure
            try:
