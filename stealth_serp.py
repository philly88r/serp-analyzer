...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("serp_stealth.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("StealthSERP")

class StealthSerpAnalyzer:
    """
    A stealth SERP analyzer that uses advanced techniques to avoid Google's anti-bot detection.
    """
    ...
    async def search_google(self, query, num_results=6):
        ...
        try:
            ...
            for attempt in range(self.max_retries):
                ...
                if self.captcha_detected:
                    logger.warning(f"CAPTCHA detected on crawler attempt {attempt+1}. Adding longer delay before retry.")
                    self.block_count += 1
    ...
    def _save_results(self, results, query):
        ...
        try:
            ...
            # Save JSON results
            json_file = f"results/serp_{query.replace(' ', '_')}_{timestamp}.json"
            with open(json_file, "w", encoding="utf-8") as f:
                json.dump(results, f, indent=2, ensure_ascii=False)
            logger.info(f"Saved JSON results to {json_file}")
            
            # Save CSV results
            csv_file = f"results/serp_{query.replace(' ', '_')}_{timestamp}.csv"
